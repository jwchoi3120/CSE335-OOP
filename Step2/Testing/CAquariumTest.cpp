...
namespace Testing
{
	...
	public:
		...
		TEST_METHOD(TestCAquariumHitTest)
		{
			...
			Assert::IsTrue(aquarium.HitTest(1000, 2000) == nullptr,
				L"Testing fish at random index");
		}
		/**
		...
		*/
		void PopulateAllTypes(CAquarium* aquarium)
		{
			...
			shared_ptr<CNeemo> fish3 = make_shared<CNeemo>(aquarium);
			fish3->SetLocation(600, 100);
			aquarium->Add(fish3);

			shared_ptr<CDecorCastle> castle = make_shared<CDecorCastle>(aquarium);
			castle->SetLocation(700, 200);
			aquarium->Add(castle);
		}
		...
		void TestAllTypes(wstring filename)
		{
			...
			// Ensure the types are correct
			Assert::IsTrue(regex_search(xml,
				wregex(L"<aqua><item.* type=\"beta\"/><item.* type=\"buddha\"/><item.* type=\"neemo\"/><item.* type=\"castle\"/></aqua>")));
			Assert::IsTrue(regex_search(xml,
				wregex(L"<item x=\"100\" y=\"200\" speedx=\"3125\\.40055543687\" speedy=\"4469\\.58830530717\"")));
			Assert::IsTrue(regex_search(xml,
				wregex(L"<item x=\"400\" y=\"400\" speedx=\"46\\.3911862544633\" speedy=\"91\\.2411877803888\"")));
			Assert::IsTrue(regex_search(xml,
				wregex(L"<item x=\"600\" y=\"100\" speedx=\"3\\.78551591540269\" speedy=\"8\\.60621967223121E-02\"")));
		}
		...
		//create aquarium, add fish, clear it, testempty
